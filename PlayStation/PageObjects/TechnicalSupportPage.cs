...
namespace PlayStation.PageObjects
{
    ...
    {
        private static Button PSStoreAndMoneyBackIcon =>
            new Button(By.XPath("//div[@class='tile__btn']/span[contains(text(),'PS Store i zwrot pieniędzy')]"));
        private static Button ReturnsIcon => new Button(By.XPath("//div[@class='tile__btn ']/span[contains(text(),'Zwroty')]"));
        private static TextElement AnswerHeader =>
            new TextElement(By.XPath("//h4[contains(text(),'Anulowanie zakupów i zarządzanie nimi')]"));
